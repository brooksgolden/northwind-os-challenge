...
Mostly swaps, because the seams are already there: drop real COGS and targets into `lib/margin.ts` (no UI change); replace the `localStorage` triage overlay with a small API + Postgres (already isolated behind one hook); flip the AI reply from the Claude-subscription provider to the Anthropic API (`DRAFT_PROVIDER=api`, the bring-your-own-key path so anyone running it uses their own account); and, once orders carry a customer id and owning rep, add the deferred New-vs-Returning split and GP-by-rep leaderboard. Then calibrate the triage weights against real win/loss data.

The bigger step is **growing triage into a real CRM**. The search and filter groundwork is already there; the next moves are a **table view** and a **Kanban board by pipeline stage**, so we can see where every lead sits in the sales process at a glance — a much better way to track things. Alongside that, **new-lead automation**: when an inquiry comes in, an LLM reads it and auto-drafts an email asking for whatever is missing from our standard intake checklist. Both depend on knowing the business better — defining our actual pipeline stages and intake fields — which is exactly why triage is deliberately basic today rather than a half-built CRM.
